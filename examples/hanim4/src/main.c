 ...
 *
 * Purpose: To illustrate H-anim Compressed Keyframes
 * *****************************************************************************/
...
static RwInt32 AnimCompressionTypeIndex;
static RwInt32 AnimReductionTypeIndex;
static RwInt32 AnimUpdateTime;
...
Initialize(void)
{
    if( RsInitialize() )
    {
        ...
        RsGlobal.appName = RWSTRING("RenderWare Graphics H-Anim Compressed Keyframes Example");
...
static RwBool 
AnimTypeCallback(RwBool justCheck)
{
    ...
    CurrentAnim = Anims[AnimReductionTypeIndex][AnimCompressionTypeIndex];

    hierarchy = GetHierarchy(Clump);
    time = hierarchy->currentAnim->currentTime;
    RpHAnimHierarchySetCurrentAnim(hierarchy, CurrentAnim);
    RpHAnimHierarchySetCurrentAnimTime(hierarchy, time);
...
InitializeMenu(void)
{    
    static const RwChar *animReductionTypeStrings[] = 
    {
        RWSTRING("original"),
        RWSTRING("duplicates"),
        RWSTRING("interpolates"),
        RWSTRING("interpolates loop"),
        RWSTRING("interpolates loop hier"),
        RWSTRING("interpolates loop hier check")
    };

    static const RwChar *animCompressionTypeStrings[] = 
    {
        RWSTRING("uncompressed"),
        RWSTRING("fixed")
    };

    static RwChar reductionLabel[] = RWSTRING("Reduction");
    static RwChar compressionLabel[] = RWSTRING("Compression");
    static RwChar fpsLabel[] = RWSTRING("FPS_F");
    

    if( MenuOpen(TRUE, &ForegroundColor, &BackgroundColor) )
    {
        MenuAddEntryInt(reductionLabel, &AnimReductionTypeIndex, 
            AnimTypeCallback, 0, NUM_REDUCTION_METHODS - 1, 1, animReductionTypeStrings);

        MenuAddEntryInt(compressionLabel, &AnimCompressionTypeIndex, 
            AnimTypeCallback, 0, NUM_COMPRESSION_METHODS - 1, 1, animCompressionTypeStrings);
...
DisplayOnScreenInfo(void)
{
    ...
    animSize = Sizes[AnimReductionTypeIndex][AnimCompressionTypeIndex] / 1024.f;

    rwsprintf(caption, RWSTRING("Current animation size: %.2fk"), animSize);
...
Idle(void)
{
    ...
    RwUInt32 animUpdateStart, animUpdateEnd;
    ...
    animUpdateStart = RsTimer();
    UpdateAnimation(Clump, deltaTime);
    animUpdateEnd = RsTimer();
    AnimUpdateTime = animUpdateEnd - animUpdateStart;
