...
Initialize(void)
{
    if( RsInitialize() )
    {
        ...
        RsGlobal.appName = RWSTRING("RenderWare Graphics Vertex Shader Example");
...
{    
    ...
    static RwChar vertexShader[] = RWSTRING("Vertex shader");

    if( MenuOpen(TRUE, &ForegroundColor, &BackgroundColor) )
    {
        MenuAddEntryBool(vertexShader, &VertexShaderOn, NULL);
        MenuAddSeparator();
        MenuAddEntryBool(fpsLabel, &FPSOn, NULL);

        /* Lets see the menu */
        MenuSetStatus(MENUMODE);
...
Initialize3D(void *param)
{
    ...
    if( !VShaderOpen())
    {
        RsErrorMessage(RWSTRING("Error initializing vertex shader."));
