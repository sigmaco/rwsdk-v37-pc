...
//these should be dependent on world BBox size (use longest axis)
// - propogate changes to all 'events.c's
...
//TODO[1] ADD EQUIVALENT CONTROL BUTTONS TO OTHER PLATFORMS
        case '+':
        case '=':
            {
                /* '+' */
                gFakeEnums[MENUSTRINGARRAYCAMSPEED] = FAKEENUMMAXVAL;
                ChangeCameraSpeed(FALSE);
                return rsEVENTPROCESSED;
            }
        ...
        case '_':
            {
                /* '-' */
                gFakeEnums[MENUSTRINGARRAYCAMSPEED] = FAKEENUMMINVAL;
                ChangeCameraSpeed(FALSE);
                return rsEVENTPROCESSED;
            }
