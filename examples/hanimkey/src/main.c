...
CreateWorld(void)
{
    ...
    bb.inf.x = bb.inf.y = bb.inf.z = -200.0f;
    bb.sup.x = bb.sup.y = bb.sup.z = 200.0f;
...
CreateCamera(RpWorld *world)
{
    ...
    {
        RwFrame *frame;
        RwV3d pos = {0.0f, 0.0f, -120.0f};

        RwCameraSetNearClipPlane(camera, 1.0f);
        RwCameraSetFarClipPlane(camera, 240.0f);
...
Initialize(void)
{
    if( RsInitialize() )
    {
        ...
        RsGlobal.appName = 
            RWSTRING("RenderWare Graphics H-Anim Custom Keyframe Interpolation Example");
...
{
    static RwChar  note[] = 
        RWSTRING("Toggle 'Translate bones' to set the bone offsets.");
    RwChar caption[256];
    ...
    RsCharsetPrint(Charset, note, 0, 0, rsPRINTPOSBOTTOMRIGHT);
