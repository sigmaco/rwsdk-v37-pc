...
CreateClump(RpWorld *world)
{
    ...
    if( clump ) 
    {
        ...
        RwFrameRotate(frame, &Yaxis, 90.0f, rwCOMBINEPOSTCONCAT);

        RwFrameRotate(frame, &Zaxis, -45.0f, rwCOMBINEPOSTCONCAT);

        RwFrameTranslate(frame, &pos, rwCOMBINEPOSTCONCAT);

        RpWorldAddClump(world, clump);

        ClumpSetRenderCallBack(clump);
    }
...
Initialize(void)
{
    if( RsInitialize() )
    {
        ...
        RsGlobal.appName = RWSTRING("RenderWare Graphics Atomic Picking Example");
...
InitializeMenu(void)
{
    static RwChar pickingLabel[] = RWSTRING("Pick method_P");

    static RwChar fpsLabel[] = RWSTRING("FPS_F");

    static const RwChar *pickMethodStrings[] = 
    {
        RWSTRING("Triangles"),
        RWSTRING("Sphere")
    };
    ...
    {
        MenuAddEntryInt(pickingLabel, (RwInt32 *)&CurrentPickMode, 
            NULL, 0, 1, 1, pickMethodStrings);
        MenuAddSeparator();
