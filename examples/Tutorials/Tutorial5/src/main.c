 ...
 *
 * Purpose: RenderWare Graphics Tutorial Five (Creating a World)
 *
 ****************************************************************************/
...
Initialize(void)
{
    if( RsInitialize() )
    {
        ...
        RsGlobal.appName = RWSTRING("RenderWare Graphics Tutorial Five");
...
saveAtomicTrigger(RwBool check)
{
    if (PickedAtomic && !check)
    {
        ...
        if (name)
        {
            ...
            RsSprintf(s, "%s.dff", name);
            DffSave(PickedAtomic, (RwChar *)s);
        }
        else
        {
            DffSave(PickedAtomic, "noname.dff");
        }
    }
    return TRUE;
}
