...
#include "pshader.h"
#include "blurvshaderD3D8.h"
#include "blurvshaderdefs.h"
#include "blurpshaderD3D8.h"
...
PShaderOpen(void)
{
    ...
    {
        status = 
            RwD3D8CreateVertexShader((RwUInt32 *)BlurVertexShaderDeclaration,
                                     (RwUInt32 *)dwBlurvshaderD3D8VertexShader,
                                     (RwUInt32 *)&BlurVertexShader,
                                     0);
    ...
    {
        RwD3D8CreatePixelShader((RwUInt32 *)dwBlurpshaderD3D8PixelShader, 
                                (RwUInt32 *)&BlurPixelShader);
    }
    else
    {
        BlurPixelShader = 0;
...
{ 
    ...
    */
    ...
    RwD3D8SetVertexShaderConstant( VSCONST_REG_T0_OFFSET,
        stageOffset, VSCONST_REG_T0_SIZE );
        
	stageOffset[0] = offsetX1;
	stageOffset[1] = offsetY1;
    RwD3D8SetVertexShaderConstant( VSCONST_REG_T0_OFFSET + VSCONST_REG_T0_SIZE,
        stageOffset, VSCONST_REG_T0_SIZE );
        
	stageOffset[0] = offsetX2;
	stageOffset[1] = offsetY2;
    RwD3D8SetVertexShaderConstant( VSCONST_REG_T0_OFFSET + 2 * VSCONST_REG_T0_SIZE,
        stageOffset, VSCONST_REG_T0_SIZE );

	stageOffset[0] = offsetX3;
	stageOffset[1] = offsetY3;
    RwD3D8SetVertexShaderConstant( VSCONST_REG_T0_OFFSET + 3 * VSCONST_REG_T0_SIZE,
        stageOffset, VSCONST_REG_T0_SIZE );
}
...
{ 
    ...
    */
    ...
    for (i = 0; i < 4; ++i)
    {
        ...
        RwD3D8SetVertexShaderConstant( VSCONST_REG_T0_OFFSET + i * VSCONST_REG_T0_SIZE,
            stageOffset, VSCONST_REG_T0_SIZE );
    }
}
...
static void Blur( int srcTexture )
{
    ...
    RwD3D8SetVertexShader( BlurVertexShader );

    RwRenderStateSet(rwRENDERSTATEZTESTENABLE, (void *) FALSE);
    RwRenderStateSet(rwRENDERSTATEZWRITEENABLE, (void *) FALSE);

    RwD3D8GetRenderState(D3DRS_CULLMODE, &cullMode);
    ...
    for (i = 0; i < 4; ++i)
    {
        RwD3D8SetTexture(CameraTexture[srcTexture], i);

            RwD3D8SetTextureStageState(i, D3DTSS_COLOROP,   D3DTOP_SELECTARG1);
            RwD3D8SetTextureStageState(i, D3DTSS_COLORARG1, D3DTA_TEXTURE);
        ...
        RwD3D8SetTextureStageState(i, D3DTSS_MAGFILTER, D3DTEXF_LINEAR);
            ...
            RwD3D8SetTextureStageState(i, D3DTSS_ADDRESSU,  D3DTADDRESS_CLAMP);
            RwD3D8SetTextureStageState(i, D3DTSS_ADDRESSV,  D3DTADDRESS_CLAMP);
    }
    RwD3D8SetTextureStageState(0, D3DTSS_COLOROP,   D3DTOP_SELECTARG1);

    /* turn on the bluring pixel shader */
    RwD3D8SetPixelShader(BlurPixelShader);      

    /* render the quad, invoking the vertex & pixel shaders */
    RwD3D8DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 4, RenderedVertex, sizeof(RenderedVertexFormat));

    /* Restore renderstates */
    for (i = 0; i < 4; ++i)
    {
        RwD3D8SetTexture(NULL, i);
    }

    RwD3D8SetRenderState(D3DRS_CULLMODE, cullMode);
...
PShaderClose(void)
{
    ...
    RwD3D8DeleteVertexShader(BlurVertexShader);

    RwD3D8DeletePixelShader(BlurPixelShader);
