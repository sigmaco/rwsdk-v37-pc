...
RwBool DumpMessage = TRUE;

static RwChar iniFile[] = RWSTRING("2dviewer.ini");
...
Initialize(void)
{
    if( RsInitialize() )
    {
        ...
        RsGlobal.appName = RWSTRING("RenderWare Graphics 2dViewer");
...
#ifdef WITH_BUTTON_DISPLAY
        CreateMenuForButtonLookUp();
#endif
        return TRUE;
    }
...
DisplayOnScreenInfo(RwCamera *camera)
{
    ...
    DisplayButtonLookUpIndices(Charset);
...
Idle(void)
{
    ...
    if(Maestro && ButtonLookUp==NULL)
    {
        CreateLookUpForAllButtons(Maestro);
        UpdateMenuForButtonLookUp();
    }
